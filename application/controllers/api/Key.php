...
class Key extends REST_Controller
{
	protected $methods = array(
		'index_put' => array('level' => 10, 'limit' => 10, 'user_afm' => 9),
 ...
 /*       $user_afm=$this->put('user_afm');
        log_message('info', 'user afm = ' .$user_afm );
        if (!$user_afm) {
            $this->response(array('status' => 0, 'error' => 'Could not find user afm. ' . $user_afm), 500); // 500 = Internal Server Error
        }*/
        $data = $this->request->body;
        echo $data['level'];

        log_message('info', 'afm = ' . $this->put('afm'));
		// If no key level provided, give them a rubbish one
		$level = $this->put('level') ? $this->put('level') : 1;
		$ignore_limits = $this->put('ignore_limits') ? $this->put('ignore_limits') : 1;

		// Insert the new key
		if (self::_insert_key($key, array('level' => $level, 'ignore_limits' => $ignore_limits, 'afm_fk' => $this->put('afm'))))
		{
